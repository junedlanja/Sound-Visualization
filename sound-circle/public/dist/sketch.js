...
const SKETCH_NAME = 'SoundCircles';
...
class Circle {
    constructor(p) {
        ...
        if (!Circle.isInitialized) {
            //set color here 
            //
            //new ShapeColor(p reference, fillColor, strokeColor, alphaEnabled)
            Circle.color = new ShapeColor(p, p.color(0, 128), p.color(0, 20), true);
            Circle.isInitialized = true;
        }
    ...
    draw() {
        ...
        if (ratio < 0.001)
            return;
        Circle.color.applyColor(145 * ratio);
        const size = this.size * ((1 - this.shrinkRatio) + this.shrinkRatio * ratio);
        //draw circle
        this.p.ellipse(this.x, this.y, size, size);
    }
}
Circle.isInitialized = false;
//CircleGenerator will responsible to generate multiple circles pattern
class CircleGenerator {
    constructor(p) {
        this.p = p;
        this.isToBeRemoved = false;
        this.isActive = false;
        this.circles = new CleanableSpriteArray(128);
        this.rotationAngle = Math.PI / 4;
        this.analyzer = new p5.Amplitude();
        this.pool = new ObjectPool$1(() => { return new Circle(p); }, 128);
    }
    step() {
        if (!this.isActive)
            return;
    ...
    generate(halfSize) {
        ...
        this.circles.push(this.pool.get().set(this, x, y, size));
    }
    clean() {
        this.circles.clean();
    }
...
const sketch = (p) => {
    // ---- constants
    const filePath = './assets/song.mpeg';
    ...
    function drawMessage() {
        if (circleGenerator.isActive && sound.isPlaying())
            return;
        p.noStroke();
        p.fill(textColor);
        const textString = circleGenerator.isActive ?
    ...
    p.setup = () => {
        ...
        circleGenerator = new CircleGenerator(p);
    };
    p.draw = () => {
        ...
        drawMessage();
        circleGenerator.step();
        circleGenerator.clean();
        circleGenerator.draw();
    };
    p.windowResized = () => {
